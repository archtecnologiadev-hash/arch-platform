...
function AceitarConviteContent() {
  ...
  if (invalid) return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 20, padding: 40, maxWidth: 400, width: '100%', textAlign: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
        <AlertCircle size={40} color="#ef4444" style={{ marginBottom: 16 }} />
        <h2 style={{ fontSize: 18, fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>Convite inválido</h2>
        <p style={{ fontSize: 13, color: 'var(--text-2)' }}>Este link não é válido ou já foi utilizado.</p>
      </div>
    </div>
  )
  ...
  if (success) return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 20, padding: 40, maxWidth: 400, width: '100%', textAlign: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
        ...
        <h2 style={{ fontSize: 18, fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>Bem-vindo à equipe!</h2>
        <p style={{ fontSize: 13, color: 'var(--text-2)' }}>Conta criada. Redirecionando para o painel...</p>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 20, padding: '36px 32px', maxWidth: 440, width: '100%', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <span style={{ fontSize: 22, fontWeight: 300, letterSpacing: '0.35em', color: 'var(--accent)' }}>ARC</span>
        </div>
        ...
        <div style={{ background: 'rgba(0,122,255,0.06)', border: '1px solid rgba(0,122,255,0.18)', borderRadius: 12, padding: '16px 18px', marginBottom: 28 }}>
          <div style={{ fontSize: 11, color: 'var(--accent)', fontWeight: 700, letterSpacing: '0.06em', marginBottom: 6 }}>CONVITE PARA EQUIPE</div>
          <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>{escritorioNome}</div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            ...
            <input value={convite?.email ?? ''} disabled style={{ ...inp, background: '#f7f7f7', color: 'var(--text-3)', cursor: 'not-allowed' }} />
          </div>
          <div>
            <label style={{ fontSize: 11, color: 'var(--text-2)', display: 'block', marginBottom: 5, fontWeight: 600 }}>Crie sua senha *</label>
          ...
          <div>
            <label style={{ fontSize: 11, color: 'var(--text-2)', display: 'block', marginBottom: 5, fontWeight: 600 }}>Confirmar senha *</label>
