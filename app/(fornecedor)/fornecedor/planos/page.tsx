...
interface Plano {
  ...
  selo_certificado: boolean
}

const FEATURES: Record<string, { label: string; included: boolean }[]> = {
  'fornecedor-vitrine': [
    { label: '10 produtos no catálogo', included: true },
    { label: 'Receber solicitações de orçamento', included: true },
    { label: 'Perfil público no marketplace', included: true },
    { label: 'Destaque no marketplace', included: false },
    { label: 'Analytics e relatórios', included: false },
    { label: 'Selo de fornecedor certificado', included: false },
  ...
  'fornecedor-destaque': [
    ...
    { label: 'Selo de fornecedor certificado', included: true },
    { label: 'Suporte prioritário', included: true },
    { label: 'Perfil com badge verificado', included: true },
  ],
}
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: planosData }, { data: { user } }] = await Promise.all([
        supabase.from('planos').select('*').eq('tipo_usuario', 'fornecedor').eq('ativo', true).order('ordem'),
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f2f2f7', padding: '36px 32px' }}>
      ...
      {planos.length === 0 ? (
        ...
        <div style={{
          display: 'grid',
          gridTemplateColumns: `repeat(${Math.min(planos.length, 3)}, minmax(280px, 380px))`,
          gap: 24, justifyContent: 'center', maxWidth: 900, margin: '0 auto',
        }}>
          {planos.map(plano => {
            const isDestaque = plano.slug === 'fornecedor-destaque'
            ...
            return (
              <div key={plano.id} style={{
                ...
                {isCurrent && !isDestaque && (
                  <div style={{
                    position: 'absolute', top: -13, left: '50%', transform: 'translateX(-50%)',
                    background: '#34d399', color: '#fff',
                    fontSize: 11, fontWeight: 700, padding: '4px 14px', borderRadius: 20, whiteSpace: 'nowrap',
                  }}>
                    Plano atual
                  </div>
                )}
                ...
                {isCurrent ? (
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: isDestaque ? '#007AFF' : 'rgba(0,122,255,0.08)',
                      color: isDestaque ? '#fff' : '#007AFF',
                      border: isDestaque ? 'none' : '1.5px solid rgba(0,122,255,0.3)',
