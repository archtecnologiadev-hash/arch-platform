...
function Avatar({ nome, url, size = 40, color = '#007AFF' }: { nome: string; url: string | null; size?: number; color?: string }) {
  return (
    <div style={{
      ...
      {url
        ? <img src={url} alt={nome} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
        : initials(nome)
      }
    </div>
  )
...
function ChatInner({ userType }: { userType: UserType }) {
  ...
  async function loadConversas(supabase: ReturnType<typeof createClient>, uid: string) {
    if (userType === 'arquiteto') {
      ...
      const { data: usersData } = await supabase
        .from('users').select('id, nome, email, avatar_url').in('id', participanteIds)
      ...
      if (fornecedorDbIds.length > 0) {
        const { data: fornsData } = await supabase
          .from('fornecedores').select('id, nome, image_url').in('id', fornecedorDbIds)
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const escMap: Record<string, any> = Object.fromEntries((escData ?? []).map(e => [e.user_id, e]))
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        setMensagens(data ?? [])
        supabase.from('mensagens').update({ lida: true })
          .eq('conversa_id', selectedId).eq('lida', false).neq('remetente_id', userId)
          .then(() => {
      ...
      <button onClick={() => setSelectedId(c.id)} style={{
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 4, marginTop: 1 }}>
            <span style={{ fontSize: 12, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flex: 1, color: c.unread > 0 ? '#1a1a1a' : '#8e8e93', fontWeight: c.unread > 0 ? 500 : 400 }}>
